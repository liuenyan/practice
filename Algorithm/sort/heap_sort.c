...
static void exchange(element_t elements[], int i, int j)
{
...
static void max_heapify(element_t elements[], int element_size, int index)
{
    ...
    if (largest != index) {
        exchange(elements, index, largest);
...
{
    make_max_heap(elements, element_size);
    for (int i = element_size; i >= 2; i--) {
        exchange(elements, 0, i - 1);
        max_heapify(elements, i - 1, 0);
    }
}
